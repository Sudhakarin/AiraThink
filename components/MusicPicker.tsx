...
export default function MusicPicker({ open, onClose, onSelect }: Props) {
  ...
  const [sourceMeta, setSourceMeta] = useState<{
    ...
    attempted: { source: string; count: number }[];
  } | null>(null);
      ...
      {trimTrack ? (
        ...
        <>
          ...
          <div className="flex-1 overflow-y-auto px-3 py-3">
            ...
            {!loading && sourceMeta && sourceMeta.usedSource && sourceMeta.usedSource !== "deezer" && (
              <p className="mb-1 px-2 text-[11px] text-amber-400">
                Deezer se kuch nahi mila — showing {sourceMeta.usedSource} results (
                {sourceMeta.attempted.map((a) => `${a.source}:${a.count}`).join(", ")})
              </p>
            )}
